    ...
    try {
      ...
      if (packages.isNotEmpty && mounted) {
        setState(() {
          ...
          final rawCoins = _featuredPackage?['total_coins'] ??
              _featuredPackage?['coins'] ??
              _featuredPackage?['amount'];
          _featuredPackageCoins = rawCoins is int
              ? rawCoins
            ...
            if (err == ErrorCodeType.errTokenExpired || err == ErrorCodeType.errInvalidToken) {
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Call session token expired or invalid.'),
